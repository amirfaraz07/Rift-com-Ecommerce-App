    ...
    <Container>
            <h1 className='w-full text-center text-[30px] text-[#1e2832] font-semibold mt-[80px] mb-[40px] uppercase'> Best Seller Products</h1>
                ...
                <Row className="items-center">
                    ...
                    {categories.map((item) => (
                    <p onClick={() => handleGetProduct(item._id)} className='text-[16px] mr-[40px] cursor-pointer text-[#00000080] hover:text-[#757575]' key={item._id}>{item.title}</p>
                    ))}
                </Row>
                
                <Row className='flex-wrap gap-[1%] mt-[40px]'>
                    { products.map( item => <div className='w-[24%] pb-[20px]' key={item._id}> 
